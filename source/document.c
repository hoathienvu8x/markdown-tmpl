...
#define MARKDOWN_LI_END 8	/* internal list flag */

const char *markdown_find_block_tag(const char *str, unsigned int len);
...
typedef size_t
(*char_trigger)(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size);

static size_t char_emphasis(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_quote(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_linebreak(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_codespan(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_escape(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_entity(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_langle_tag(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_autolink_url(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_autolink_email(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_autolink_www(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_link(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_image(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_superscript(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_math(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size);
...
struct markdown_document {
	...
	markdown_renderer_data data;

	struct link_ref *refs[REF_TABLE_SIZE];
	struct footnote_list footnotes_found;
	struct footnote_list footnotes_used;
	uint8_t active_char[256];
	markdown_stack work_bufs[2];
	markdown_extensions ext_flags;
	size_t max_nesting;
	int in_link_body;
};
...
static markdown_buffer *
newbuf(markdown_document *doc, int type)
{
	static const size_t buf_size[2] = {256, 64};
	markdown_buffer *work = NULL;
	markdown_stack *pool = &doc->work_bufs[type];
	...
	if (pool->size < pool->asize &&
		...
		work = markdown_buffer_new(buf_size[type]);
		markdown_stack_push(pool, work);
	}
...
static void
popbuf(markdown_document *doc, int type)
{
	doc->work_bufs[type].size--;
}

static void
unscape_text(markdown_buffer *ob, markdown_buffer *src)
{
	size_t i = 0, org;
	while (i < src->size) {
		org = i;
		...
		if (i > org)
			markdown_buffer_put(ob, src->data + org, i - org);

		if (i + 1 >= src->size)
			break;

		markdown_buffer_putc(ob, src->data[i + 1]);
		i += 2;
	}
}
...
{
	struct link_ref *ref = markdown_calloc(1, sizeof(struct link_ref));
...
free_link_refs(struct link_ref **references)
{
	...
	for (i = 0; i < REF_TABLE_SIZE; ++i) {
		...
		while (r) {
			next = r->next;
			markdown_buffer_free(r->link);
			markdown_buffer_free(r->title);
			free(r);
			r = next;
		}
	}
...
{
	struct footnote_ref *ref = markdown_calloc(1, sizeof(struct footnote_ref));
...
{
	struct footnote_item *item = markdown_calloc(1, sizeof(struct footnote_item));
	if (!item)
		return 0;
	item->ref = ref;
...
{
	markdown_buffer_free(ref->contents);
	free(ref);
}
 ...
 */
static void
replace_spacing(markdown_buffer *ob, const uint8_t *data, size_t size)
{
	size_t i = 0, mark;
	markdown_buffer_grow(ob, size);
	while (1) {
		mark = i;
		while (i < size && data[i] != '\n') i++;
		markdown_buffer_put(ob, data + mark, i - mark);
...
static size_t
tag_length(uint8_t *data, size_t size, markdown_autolink_type *autolink)
{
	size_t i, j;
	...
	/* scheme test */
	*autolink = MARKDOWN_AUTOLINK_NONE;
	...
	if (i > 1 && data[i] == '@') {
		if ((j = is_mail_autolink(data + i, size - i)) != 0) {
			*autolink = MARKDOWN_AUTOLINK_EMAIL;
			return i + j;
		}
	}

	if (i > 2 && data[i] == ':') {
		*autolink = MARKDOWN_AUTOLINK_NORMAL;
		i++;
	}
...
static void
parse_inline(hoedown_buffer *ob, markdown_document *doc, uint8_t *data, size_t size)
{
	size_t i = 0, end = 0, consumed = 0;
	markdown_buffer work = { 0, 0, 0, 0, NULL, NULL, NULL };
	...
	while (i < size) {
		...
		if (doc->md.normal_text) {
			...
			markdown_buffer_put(ob, data + i, end - i);
...
static size_t
parse_emph1(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t size, uint8_t c)
{
	size_t i = 0, len;
	markdown_buffer *work = 0;
	int r;
	...
	while (i < size) {
		...
		if (data[i] == c && !_isspace(data[i - 1])) {
			...
			if (doc->ext_flags & MARKDOWN_EXT_NO_INTRA_EMPHASIS) {
			...
			if (doc->ext_flags & MARKDOWN_EXT_UNDERLINE && c == '_')
...
static size_t
parse_emph2(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t size, uint8_t c)
{
	size_t i = 0, len;
	markdown_buffer *work = 0;
	int r;
...
static size_t
parse_emph3(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t size, uint8_t c)
{
	size_t i = 0, len;
	int r;
	...
	while (i < size) {
		...
		if (i + 2 < size && data[i + 1] == c && data[i + 2] == c && doc->md.triple_emphasis) {
			/* triple symbol found */
			markdown_buffer *work = newbuf(doc, BUFFER_SPAN);
...
static size_t
parse_math(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size, const char *end, size_t delimsz, int displaymode)
{
	markdown_buffer text = { NULL, 0, 0, 0, NULL, NULL, NULL };
	size_t i = delimsz;
...
static size_t
char_emphasis(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	uint8_t c = data[0];
	size_t ret;
...
static size_t
char_linebreak(markdown_buffer *ob, markedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
...
static size_t
char_codespan(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	markdown_buffer work = { NULL, 0, 0, 0, NULL, NULL, NULL };
...
static size_t
char_quote(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size)
{
...
static size_t
char_escape(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	...
	if (size > 1) {
		...
		else markdown_buffer_putc(ob, data[1]);
	} else if (size == 1) {
		if (doc->md.normal_text) {
			work.data = data;
			work.size = 1;
		...
		else markdown_buffer_putc(ob, data[0]);
	}
...
static size_t
char_entity(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	size_t end = 1;
	markdown_buffer work = { 0, 0, 0, 0, NULL, NULL, NULL };
	...
	else markdown_buffer_put(ob, data, end);
...
static size_t
char_langle_tag(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	markdown_buffer work = { NULL, 0, 0, 0, NULL, NULL, NULL };
	markdown_autolink_type altype = MARKDOWN_AUTOLINK_NONE;
	...
	if (end > 2) {
		if (doc->md.autolink && altype != MARKDOWN_AUTOLINK_NONE) {
			markdown_buffer *u_link = newbuf(doc, BUFFER_SPAN);
			work.data = data + 1;
			work.size = end - 2;
...
static size_t
char_autolink_www(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	markdown_buffer *link, *link_url, *link_text;
	...
	if ((link_len = markdown_autolink__www(&rewind, link, data, offset, size, MARKDOWN_AUTOLINK_SHORT_DOMAINS)) > 0) {
		link_url = newbuf(doc, BUFFER_SPAN);
		MARKDOWN_BUFPUTSL(link_url, "http://");
		markdown_buffer_put(link_url, link->data, link->size);
...
static size_t
char_autolink_email(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	markdown_buffer *link;
	size_t link_len, rewind;
	...
	if ((link_len = markdown_autolink__email(&rewind, link, data, offset, size, 0)) > 0) {
		if (ob->size > rewind)
			ob->size -= rewind;
		else
			ob->size = 0;

		doc->md.autolink(ob, link, MARKDOWN_AUTOLINK_EMAIL, &doc->data);
	}
...
static size_t
char_autolink_url(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	markdown_buffer *link;
	size_t link_len, rewind;
	...
	if ((link_len = markdown_autolink__url(&rewind, link, data, offset, size, 0)) > 0) {
		if (ob->size > rewind)
			ob->size -= rewind;
		else
			ob->size = 0;

		doc->md.autolink(ob, link, MARKDOWN_AUTOLINK_NORMAL, &doc->data);
	}
...
static size_t
char_image(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size) {
	size_t ret;
...
static size_t
char_link(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	int is_img = (offset && data[-1] == '!' && !is_escaped(data - offset, offset - 1));
	int is_footnote = (doc->ext_flags & MARKDOWN_EXT_FOOTNOTES && data[1] == '^');
	size_t i = 1, txt_e, link_b = 0, link_e = 0, title_b = 0, title_e = 0;
	markdown_buffer *content = NULL;
	markdown_buffer *link = NULL;
	markdown_buffer *title = NULL;
	markdown_buffer *u_link = NULL;
	...
	/* footnote link */
	if (is_footnote) {
		markdown_buffer id = { NULL, 0, 0, 0, NULL, NULL, NULL };
	...
	if (i < size && data[i] == '(') {
		...
		if (link_e > link_b) {
			link = newbuf(doc, BUFFER_SPAN);
			markdown_buffer_put(link, data + link_b, link_e - link_b);
		}

		if (title_e > title_b) {
			title = newbuf(doc, BUFFER_SPAN);
			markdown_buffer_put(title, data + title_b, title_e - title_b);
		}
	...
	/* reference style link */
		...
		markdown_buffer *id = newbuf(doc, BUFFER_SPAN);
		struct link_ref *lr;
		...
		if (link_b == link_e)
			...
			markdown_buffer_put(id, data + link_b, link_e - link_b);
	...
	if (txt_e > 1) {
		...
		if (is_img) {
			markdown_buffer_put(content, data + 1, txt_e - 1);
		} else {
...
static size_t
char_superscript(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size)
{
...
static size_t
char_math(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	/* double dollar */
	if (size > 1 && data[1] == '$')
	...
	if (doc->ext_flags & MARKDOWN_EXT_MATH_EXPLICIT)
...
static size_t
parse_codefence(uint8_t *data, size_t size, markdown_buffer *lang, size_t *width, uint8_t *chr)
{
...
static int
is_atxheader(markdown_document *doc, uint8_t *data, size_t size)
{
	if (data[0] != '#')
		return 0;

	if (doc->ext_flags & MARKDOWN_EXT_SPACE_HEADERS) {
		size_t level = 0;
...
static void parse_block(markdown_buffer *ob, markdown_document *doc,
			uint8_t *data, size_t size);


/* parse_blockquote • handles parsing of a blockquote fragment */
static size_t
parse_blockquote(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t size)
{
	size_t beg, end = 0, pre, work_size = 0;
	uint8_t *work_data = 0;
	markdown_buffer *out = 0;
...
static size_t
parse_htmlblock(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t size, int do_render);

/* parse_blockquote • handles parsing of a regular paragraph */
static size_t
parse_paragraph(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t size)
{
	markdown_buffer work = { NULL, 0, 0, 0, NULL, NULL, NULL };
	size_t i = 0, end = 0;
	int level = 0;
...
static size_t
parse_fencedcode(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t size)
{
	markdown_buffer text = { 0, 0, 0, 0, NULL, NULL, NULL };
	markdown_buffer lang = { 0, 0, 0, 0, NULL, NULL, NULL };
...
static size_t
parse_blockcode(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t size)
{
	size_t beg, end, pre;
	markdown_buffer *work = 0;
	...
	while (beg < size) {
		...
		if (beg < end) {
			...
			if (is_empty(data + beg, end - beg))
				markdown_buffer_putc(work, '\n');
			else markdown_buffer_put(work, data + beg, end - beg);
		}
		beg = end;
	}
...
static size_t
parse_listitem(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t size, markdown_list_flags *flags)
{
	markdown_buffer *work = 0, *inter = 0;
	...
	markdown_buffer_put(work, data + beg, end - beg);
	beg = end;
	...
	while (beg < size) {
		...
		if (doc->ext_flags & MARKDOWN_EXT_FENCED_CODE) {
		...
		if ((has_next_uli && !is_hrule(data + beg + i, end - beg - i)) || has_next_oli) {
			...
			if (pre <= orgpre) {
				...
				if (in_empty && (
					((*flags & MARKDOWN_LIST_ORDERED) && has_next_uli) ||
					(!(*flags & MARKDOWN_LIST_ORDERED) && has_next_oli)))
					*flags |= MARKDOWN_LI_END;
		...
		markdown_buffer_put(work, data + beg + i, end - beg - i);
		beg = end;
	}

	/* render of li contents */
	if (has_inside_empty)
		*flags |= MARKDOWN_LI_BLOCK;

	if (*flags & MARKDOWN_LI_BLOCK) {
...
static size_t
parse_list(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t size, markdown_list_flags flags)
{
	markdown_buffer *work = 0;
	size_t i = 0, j;
...
static size_t
parse_atxheader(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t size)
{
	size_t level = 0;
	size_t i, end, skip;
...
static void
parse_footnote_def(markdown_buffer *ob, markdown_document *doc, unsigned int num, uint8_t *data, size_t size)
{
	markdown_buffer *work = 0;
	work = newbuf(doc, BUFFER_SPAN);
...
static void
parse_footnote_list(markdown_buffer *ob, markdown_document *doc, struct footnote_list *footnotes)
{
	markdown_buffer *work = 0;
...
parse_htmlblock(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t size, int do_render)
{
	...
	if (i < size)
		curtag = markdown_find_block_tag((char *)data + 1, (int)i - 1);
...
parse_table_row(
	...
	size_t size,
	size_t columns,
	markdown_table_flags *col_data,
	markdown_table_flags header_flag)
{
	size_t i = 0, col, len;
	markdown_buffer *row_work = 0;
	...
	for (; col < columns; ++col) {
		markdown_buffer empty_cell = { 0, 0, 0, 0, NULL, NULL, NULL };
...
parse_table_header(
	...
	size_t size,
	size_t *columns,
	markdown_table_flags **column_data)
{
	int pipes;
	...
	*columns = pipes + 1;
	*column_data = markdown_calloc(*columns, sizeof(markdown_table_flags));
	...
	for (col = 0; col < *columns && i < under_end; ++col) {
		...
		if (data[i] == ':') {
			i++; (*column_data)[col] |= MARKDOWN_TABLE_ALIGN_LEFT;
			dashes++;
		}
		...
		if (i < under_end && data[i] == ':') {
			i++; (*column_data)[col] |= MARKDOWN_TABLE_ALIGN_RIGHT;
			dashes++;
		}
	...
	size_t size)
{
	...
	markdown_buffer *work = 0;
	markdown_buffer *header_work = 0;
	markdown_buffer *body_work = 0;

	size_t columns;
	markdown_table_flags *col_data = NULL;
...
/* parse_block • parsing of one block, returning next uint8_t to parse */
static void
parse_block(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t size)
{
	size_t beg, end, i;
	uint8_t *txt_data;
	beg = 0;
	...
	while (beg < size) {
		...
		if (is_atxheader(doc, txt_data, end))
		...
		else if ((doc->ext_flags & MARKDOWN_EXT_FENCED_CODE) != 0 &&
			(i = parse_fencedcode(ob, doc, txt_data, end)) != 0)
			beg += i;

		else if ((doc->ext_flags & MARKDOWN_EXT_TABLES) != 0 &&
		...
		else if (!(doc->ext_flags & MARKDOWN_EXT_DISABLE_INDENTED_CODE) && prefix_code(txt_data, end))
			...
			beg += parse_list(ob, doc, txt_data, end, MARKDOWN_LIST_ORDERED);
...
is_footnote(const uint8_t *data, size_t beg, size_t end, size_t *last, struct footnote_list *list)
{
	...
	contents = markdown_buffer_new(64);
	...
	while (i < end) {
		...
		if (ind == 0) {
			...
			markdown_buffer_putc(contents, '\n');
		}

		in_empty = 0;

		/* adding the line into the content buffer */
		markdown_buffer_put(contents, data + start + ind, i - start - ind);
		/* add carriage return */
		if (i < end) {
			markdown_buffer_putc(contents, '\n');
			if (i < end && (data[i] == '\n' || data[i] == '\r')) {
...
/*	int n; */
	...
	if (refs) {
		...
		ref->link = markdown_buffer_new(link_end - link_offset);
		markdown_buffer_put(ref->link, data + link_offset, link_end - link_offset);

		if (title_end > title_offset) {
			ref->title = markdown_buffer_new(title_end - title_offset);
			markdown_buffer_put(ref->title, data + title_offset, title_end - title_offset);
		}
	}
...
static void expand_tabs(markdown_buffer *ob, const uint8_t *line, size_t size)
{
	...
	while (i < size) {
		...
		if (i > org)
			markdown_buffer_put(ob, line + org, i - org);

		if (i >= size)
			break;

		do {
			markdown_buffer_putc(ob, ' '); tab++;
		} while (tab % 4);
...
markdown_document *
markdown_document_new(
	const markdown_renderer *renderer,
	markdown_extensions extensions,
	size_t max_nesting)
{
	markdown_document *doc = NULL;

	assert(max_nesting > 0 && renderer);

	doc = markdown_malloc(sizeof(markdown_document));
	memcpy(&doc->md, renderer, sizeof(markdown_renderer));

	doc->data.opaque = renderer->opaque;

	markdown_stack_init(&doc->work_bufs[BUFFER_BLOCK], 4);
	markdown_stack_init(&doc->work_bufs[BUFFER_SPAN], 8);

	memset(doc->active_char, 0x0, 256);

	if (extensions & MARKDOWN_EXT_UNDERLINE && doc->md.underline) {
	...
	if (doc->md.emphasis || doc->md.double_emphasis || doc->md.triple_emphasis) {
		...
		doc->active_char['_'] = MD_CHAR_EMPHASIS;
		if (extensions & MARKDOWN_EXT_STRIKETHROUGH)
			doc->active_char['~'] = MD_CHAR_EMPHASIS;
		if (extensions & MARKDOWN_EXT_HIGHLIGHT)
			doc->active_char['='] = MD_CHAR_EMPHASIS;
	}
...
void
markdown_document_render(markdown_document *doc, markdown_buffer *ob, const uint8_t *data, size_t size)
{
	...
	text = markdown_buffer_new(64);

	/* Preallocate enough space for our buffer to avoid expanding while copying */
	markedown_buffer_grow(text, size);

	/* reset the references table */
	memset(&doc->refs, 0x0, REF_TABLE_SIZE * sizeof(void *));

	footnotes_enabled = doc->ext_flags & MARKDOWN_EXT_FOOTNOTES;
	...
	markdown_buffer_grow(ob, text->size + (text->size >> 1));
...
void
markdown_document_render_inline(markdown_document *doc, markdown_buffer *ob, const uint8_t *data, size_t size)
{
	size_t i = 0, mark;
	markdown_buffer *text = markdown_buffer_new(64);

	/* reset the references table */
	memset(&doc->refs, 0x0, REF_TABLE_SIZE * sizeof(void *));

	/* first pass: expand tabs and process newlines */
	markdown_buffer_grow(text, size);
	while (1) {
		mark = i;
		while (i < size && data[i] != '\n' && data[i] != '\r')
			i++;
...
void
markdown_document_free(markdown_document *doc)
{
	size_t i;

	for (i = 0; i < (size_t)doc->work_bufs[BUFFER_SPAN].asize; ++i)
		markdown_buffer_free(doc->work_bufs[BUFFER_SPAN].item[i]);

	for (i = 0; i < (size_t)doc->work_bufs[BUFFER_BLOCK].asize; ++i)
		markdown_buffer_free(doc->work_bufs[BUFFER_BLOCK].item[i]);

	markdown_stack_uninit(&doc->work_bufs[BUFFER_SPAN]);
	markdown_stack_uninit(&doc->work_bufs[BUFFER_BLOCK]);
