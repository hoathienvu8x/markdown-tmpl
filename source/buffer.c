...
void
markdown_buffer_uninit(hoedown_buffer *buf)
{
	assert(buf && buf->unit);
...
int
markdown_buffer_putf(markdown_buffer *buf, FILE *file)
{
	assert(buf && buf->unit);

	while (!(feof(file) || ferror(file))) {
		markdown_buffer_grow(buf, buf->size + buf->unit);
...
void markdown_buffer_put_utf8(markdown_buffer *buf, unsigned int c) {
	unsigned char unichar[4];

	assert(buf && buf->unit);

	if (c < 0x80) {
		markdown_buffer_putc(buf, c);
	}
	else if (c < 0x800) {
		unichar[0] = 192 + (c / 64);
		unichar[1] = 128 + (c % 64);
		markdown_buffer_put(buf, unichar, 2);
	}
	else if (c - 0xd800u < 0x800) {
		MARKDOWN_BUFPUTSL(buf, "\xef\xbf\xbd");
	}
	else if (c < 0x10000) {
		unichar[0] = 224 + (c / 4096);
		unichar[1] = 128 + (c / 64) % 64;
		unichar[2] = 128 + (c % 64);
		markdown_buffer_put(buf, unichar, 3);
	}
	else if (c < 0x110000) {
		...
		unichar[3] = 128 + (c % 64);
		markdown_buffer_put(buf, unichar, 4);
	}
	else {
		MARKDOWN_BUFPUTSL(buf, "\xef\xbf\xbd");
	}
