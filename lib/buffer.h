...
/* markdown_buffer_uninit: uninitialize an existing buffer */
void markdown_buffer_uninit(markdown_buffer *buf);
...
/* markdown_buffer_putf: read from a file and append to a buffer, until EOF or error */
int markdown_buffer_putf(markdown_buffer *buf, FILE* file);
...
/* markdown_buffer_put_utf8: put a Unicode character encoded as UTF-8 */
void markdown_buffer_put_utf8(markdown_buffer *buf, unsigned int codepoint);
...
/* MARKDOWN_BUFSETSL: optimized hoedown_buffer_sets of a string literal */
#define MARKDOWN_BUFSETSL(output, literal) \
	markdown_buffer_set(output, (const uint8_t *)literal, sizeof(literal) - 1)

/* MARKDOWN_BUFEQSL: optimized hoedown_buffer_eqs of a string literal */
